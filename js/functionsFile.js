...
/*async*/ function newGameGenerate(){ 
    ...
    gameField[5][6].contains = new Unit(peasant,1);
    gameField[5][5].contains = new Unit(peasant,1);
    gameField[4][5].contains = new Unit(scout2,1);
    //gameField[1][1].contains = townhall;
    gameField[2][3].contains = new Unit(peasant,2);
    gameField[2][1].contains = new Unit(peasant,2);
    gameField[1][1].contains = new Unit(peasant,2);
...
function unlockCells(count,i,j, type){
    if(type == 'move'){
        ...
        if (count>0){
            if (gameField[i+1][j].contains != undefined && gameField[i+1][j].contains.owner != gameSettings.turnOwner){gameField[i+1][j].availability = true;document.getElementById(`${i+1}-${j}`).style.border = colorCursor[1];}
            if (gameField[i][j+1].contains != undefined && gameField[i][j+1].contains.owner != gameSettings.turnOwner){gameField[i][j+1].availability = true;document.getElementById(`${i}-${j+1}`).style.border = colorCursor[1];}
            if (gameField[i-1][j].contains != undefined && gameField[i-1][j].contains.owner != gameSettings.turnOwner){gameField[i-1][j].availability = true;document.getElementById(`${i-1}-${j}`).style.border = colorCursor[1];}
            if (gameField[i][j-1].contains != undefined && gameField[i][j-1].contains.owner != gameSettings.turnOwner){gameField[i][j-1].availability = true;document.getElementById(`${i}-${j-1}`).style.border = colorCursor[1];}
